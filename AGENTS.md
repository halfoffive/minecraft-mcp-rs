...
- **`shutdown_token` 生命周期约定:** `SharedState` 持有 `CancellationToken`（来自 `tokio-util`），通过 `shutdown_token() -> CancellationToken` 暴露、`trigger_shutdown()` 触发。`MinecraftApp::drop` 调用 `trigger_shutdown()`；`serve_http` 用 `axum::serve(...).with_graceful_shutdown(state.shutdown_token().cancelled())` 监听；`serve_stdio` 用 `tokio::select!` 并联 `shutdown_token.cancelled()` 与 `running.waiting()`，shutdown 触发时立即返回不等 stdin EOF。MCP 线程 `JoinHandle` 现存于 `MinecraftApp`，drop 时 join（3 秒超时，与 bot 线程一致）。
