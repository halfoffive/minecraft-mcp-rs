...
二进制接受少量命令行参数（由 clap 解析，帮助文本由参数文档自动生成并输出到 stderr）：
...
| `-V`, `--version` | 打印版本号到 stderr 并退出 0 |

优先级规则不变：`--headless` 优先于 `--gui`；`--stdio` 单独使用隐含无头；
`--config <path>` 单独使用运行 GUI。无参数时打印帮助并退出 0；用法错误输出到
stderr 并退出 2。
