...
//! bare `--config <path>` resolves to the GUI, and repeated flags are
...
pub struct CliArgs {
    ...
    #[arg(long)]
    pub gui: bool,
    /// Load the config file at <PATH> instead of the OS config dir.
...
impl CliArgs {
    ...
    pub fn is_empty(&self) -> bool {
        !self.headless && !self.force_stdio && !self.gui && self.config_path.is_none()
    }
}
...
/// (or `--stdio` *without* `--gui`) forces headless; anything else —
/// including `--config` alone — runs the GUI. `--gui` beats the
/// `--stdio`-alone implication, so `--gui --stdio` still opens the window.
...
/// stderr and exits 0. Usage errors (unknown flag, missing `--config` value)
/// surface as clap errors; the caller prints them to stderr and exits 2.
...
pub fn print_help() {
    ...
    eprint!("{}", CliArgs::command().render_help());
}
...
mod tests {
    ...
    /// No arguments → defaults (all flags false, no config).
    #[test]
    ...
    /// `--config` at the end of the args with no value is a usage error.
    #[test]
    ...
    fn test_all_flags_combined() {
        let args = parse(&["--headless", "--stdio", "--config", "cfg.json"]).unwrap();
        assert!(args.headless);
        assert!(args.force_stdio);
